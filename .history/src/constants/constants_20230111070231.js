export const projects = [
  {
    title: "Movies App",
    description:
      "Using Angular, Bootstrap, and TMDB (API source) built a movie website with multiple pages that have pagination, functional search bar and other awesome feature",
    image: "/images/1.png",
    tags: ["Mongo", "Express", "React", "Node"],
    source: "https://github.com/La-Phoenix/Memories",
    visit: "https://google.com",
    id: 0,
  },
  {
    title: "MERN Memories",
    description:
      "Using React, Node.js, Express & MongoDB, built a Full Stack MERN Application; a simple social media app that allows users to post interesting events that happened in their lives.",
    image: "/images/1.png",
    tags: ["Mongo", "Express", "React", "Node"],
    source: "https://github.com/La-Phoenix/Memories",
    visit: "https://google.com",
    id: 0,
  },
  {
    title: "YelpCamp",
    description:
      "Using Node.js, Express, MongoDB and EJS, built an app with authorization and authentication that allows users to shared images of their favorite places. Also allows authenticated users to leave reviews.",
    image: "/images/Campground4.png",
    tags: ["Node", "Express", "EJS", "MongoDB"],
    source: "https://github.com/La-Phoenix/YelpCamp",
    visit: "https://google.com",
    id: 1,
  },
  {
    title: "WebShopApp",
    description:
      "Using Angular and TypeScript built an ecommerces app with stripe payment system",
    image: "/images/WebshopApp.png",
    tags: ["Angular", "TypeScript", "Stripe"],
    source: "https://github.com/La-Phoenix/WepShopApp",
    visit: "https://google.com",
    id: 2,
  },
  // {
  //   title: "Drag and Drop",
  //   description:
  //     "Using Angular, TypeScript, Node, Express and PostgreSQL, built an app with authorization and authentication that allows users to select tasks through drag and drop and allows admins to create users and assign tasks to users categories(App is a company project, no active link or source code included)",
  //   image: "/images/4.jpg",
  //   tags: ["Angular", "Typescript", "Node", "PostgreSQL", "Express"],
  //   source: "https://google.com",
  //   visit: "https://google.com",
  //   id: 3,
  // },
  // {
  //   title: "Tracker",
  //   description:
  //     "Using React, Node, Express and PostgreSQL, built an app that lists states and cities of selected countries.(App is a company project, no active link or source code included)",
  //   image: "/images/4.jpg",
  //   tags: ["React", "Node", "PostgreSQL", "Express"],
  //   source: "https://google.com",
  //   visit: "https://google.com",
  //   id: 3,
  // },
  {
    title: "MERN App",
    description:
      "Using React, Node, Express and PostgreSQL, built an that allows users to share images with of favorite places. With authentication and authorization.",
    image: "/images/MERNApp.png",
    tags: ["React", "Node", "MongoDB", "Express"],
    source: "https://github.com/La-Phoenix/MERN-App",
    visit: "https://google.com",
    id: 3,
  },
];

export const TimeLineData = [
  {
    year: "2022(february)",
    text: "Started my Web Development BootCamp on Udemy.",
  },
  {
    year: "August",
    text: "Finished my 64 Hour course with a certificate on Udemy.",
  },
  { year: "September", text: "Started working with React.js." },
  { year: "November", text: "Started working with Angular and PostgreSQL." },
  { year: "November", text: "Started Intership at Ithena.ai." },
];
